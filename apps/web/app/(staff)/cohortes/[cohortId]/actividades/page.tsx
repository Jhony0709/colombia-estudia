 ...
 *
 * Una lista y, al lado, la entrega abierta (`?actividad=<id>`): se lee el texto, se descarga
...
function hrefFor(cohortId: string, f: Partial<Filters>): string {
  ...
  if (f.open) search.set('actividad', f.open);
  return `/cohortes/${cohortId}/actividades?${search.toString()}`;
}
  ...
  return (
    <Page wide>
      ...
      <WithRail
        ...
        rail={
          open ? (
            <RailCard id="actividad-abierta" title={t('rail.openTitle')} icon={Eye} tone="info">
              <div className="space-y-3">
                <div>
                  ...
                  <div className="mt-1">
                    <StatusBadge domain="submission" status={open.status} />
                  </div>
                </div>

                {/* Lo que se pidió, plegado: quien revisa muchas seguidas ya lo sabe. */}
                {open.activityInstructions && (
                  <details className="type-body">
                    ...
                    <p className="type-body text-text-muted mt-2 max-h-60 overflow-y-auto whitespace-pre-wrap">
                      {open.activityInstructions}
                    </p>
                  </details>
                )}
            ...
            <RailCard id="actividades-como" title={t('rail.howTitle')} icon={Info}>
        ...
        <DataTable<SubmissionRow>
          ...
          columns={[
            ...
            },
            {
              ...
              header: t('status'),
              narrow: true,
              cell: (r) => <StatusBadge domain="submission" status={r.status} />,
            },
            {
              key: 'submittedAt',
