        ...
        back={
          <Breadcrumb
            ...
            items={[
              ...
              { label: tc('assessments'), href: '/contenido/evaluaciones' },
              { label: header.title },
            ]}
          />
        }
      ...
      <PageSection id="ajustes" title={t('settingsTitle')} card>
        <div className="flex flex-wrap gap-4">
          ...
          <SelectField
            ...
            value={reviewPolicy}
            options={['NONE', 'SCORE_ONLY', 'FULL_AFTER_GRADED', 'FULL_AFTER_DUE'].map((value) => ({
              value,
              ...
              <ul className="divide-border-muted -my-1 divide-y">
                {globalIssues.map((issue, index) => (
                  <li key={index} className="space-y-1 py-3">
                    ...
                    {issue.text.fix !== null && (
                      <p className="type-caption text-text-muted max-w-reading">{issue.text.fix}</p>
                    )}
